...
async fn main() {
    ...
    if let Ok(_url) = std::env::var("LOCAL_DYNAMODB_URL") {
        library::dynamodb::create_table_if_not_exists(&db_client, table_name).await;
    }
    let _ = library::model::user::create(&db_client, table_name, preferred_username.as_str()).await;
}
